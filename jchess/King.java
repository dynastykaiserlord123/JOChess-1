		...
		if (!this.wasMotion && !this.isChecked()) {// check if king was not moved before
			...
			if (chessboard.squares[0][this.square.pozY].piece != null
				...
				if (!rook.wasMotion) {
					...
					if (canCastling && this.isSafe(sq) && this.isSafe(sq1)) { // can do castling when none of Sq,sq1 is
																				// checked
						list.add(sq);
					}
				}
...
//									}									
	...
	private boolean isSafe(Square s) // A bit confusing code.
	{
		...
		{
			if (this.chessboard.squares[h][i].piece == null || this.chessboard.squares[h][i].piece == this) // if on
																											// this
																											// sqhuare
																											// isn't
																											// piece
		...
		// Pawn
		// if (false) //check if player "go" down or up
		// {//System.out.println("go down");
		// newX = s.pozX - 1;
		// newY = s.pozY + 1;
		// if (!isout(newX, newY))
		// {
		// if (this.chessboard.squares[newX][newY].piece == null) //if on this sqhuare
		// isn't piece
		// {
		// }
		// else if (this.chessboard.squares[newX][newY].piece.player == this.player)
		// //if is our piece
		// {
		// }
